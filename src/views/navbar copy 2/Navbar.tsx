...
const getSectionPos = (scrollMaxHeight: number) => {
	...
	navItems.map((item: string) => {
		...
		return (sectionPos[item] = {
			per: Math.ceil((element.getBoundingClientRect().y * 100) / scrollMaxHeight)
		})
	})
}
...
const getItemsPos = (compass: any, compassBody: any) => {
	...
	navItems.map((item: string, i) => {
		...
		if (previewItem) {
			...
			sectionPos[previewItem].differenceP = (sectionPos[item].per - sectionPos[previewItem].per) / 100
		}
		...
		if (i === navItems.length - 1) {
			...
			const endLineMidPos = endLineElementSizes.x + endLineElementSizes.width / 2

			sectionPos[item].difference = (endLineMidPos - sectionPos[item].midPos) / 100
...
const Navbar = () => {
	...
	const setCompassItemPos = () => {
		const itemMidScrollPos =
			sectionPos[selectedItem].midPos - sectionPos.compass.mid + compass.current.scrollLeft

		compass.current.scrollLeft =
			itemMidScrollPos +
			((scrollPos - sectionPos[selectedItem].per) / sectionPos[selectedItem].differenceP) *
	...
	useEffect(() => {
		...
		window.addEventListener("scroll", () => {
			...
			navItems.map((item: string, index: number) => {
				if (
					(index === navItems.length - 1 && scrollPos >= sectionPos[item].per) ||
			...
			<div className="grid grid-cols-[15%_70%_15%] h-16 items-center max-w-7xl mx-auto">
				<div className="flex justify-start items-center h-full">
					<Title text="Nanite Systems" titleOffset={60} direction={["down", "right"]}>
						<div className="relative bg-logo w-12 h-12 mt-1 bg-contain bg-no-repeat -skew-x-[25deg] brightness-0 invert ml-4"></div>
					</Title>
				</div>
				<div
					id="compassWrapper"
					ref={compassWrapper}
					className="relative w-full min-h-[32px] animate-blinkX2X"
				>
					<div
						id="compass"
              ...
              after:absolute after:-top-1 after:-right-2 after:w-5 after:h-12 after:border-4 after:border-l-0 dark:after:border-orange-500"
						...
						>
							<div ref={compassBody} className="flex">
								{compassItems.map((item, index) => {
									return (
										<div
											id={"nav-" + item + (intermediate.includes(item) ? "-" + index : "")}
											key={item + "-" + index}
                        ...
                        ${
										...
										>
											{item === "dot" ? (
												...
												<Icon type="Minus" size={30} className="rotate-90" />
											) : (
				...
				<div className="flex justify-end items-center">
					...
					<Title text="Dark theme" titleOffset={60} direction={["down", "left"]}>
						<button id="toggle" className="p-2" onClick={() => toggleTheme()}>
							<Icon
