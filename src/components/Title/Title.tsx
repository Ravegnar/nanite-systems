...
const Title = ({
  ...
  useEffect(() => {
    ...
    ctx.beginPath();
    ctx.moveTo(0, 0);
    ctx.lineTo(titleOffset, titleOffset);
    ...
    setElement(titleBody, "width", titleWidth +"px", 200)
  }, [showTooltip]);
      ...
      {showTooltip && (
        <>
          <Icon
            ...
            size={16}
            className={`absolute top-full ${xDirection}-full flex-nowrapXXX text-white`}
          />
          <canvas
            ref={canvas}
            width={titleOffset + "px"}
            height={titleOffset + "px"}
            className={`absolute inline-block w-auto -bottom-[13px] -${xDirection}-[13px] translate-y-full translate-x-full flex-nowrapXXX text-white`}
          ></canvas>
          <div
            ref={titleBody}
            style={{
              bottom: -(titleOffset - 28 + 13) + "px",
              right: xDirection === "right" ? -(titleOffset - 1) + "px" : "none",
              left: xDirection === "left" ? -(titleOffset - 1) + "px" : "none",
            }}
            className={`absolute w-0 flex h-7 whitespace-nowrap bg-cyan-600/50 text-black translate-y-full translate-x-full
              skew-x-[45deg] border-l-[10px] border-b-[2px] border-white animate-blinkX2X transition-all transform duration-75 ease-in 
            `}
          >
            <div ref={titleText} className="flex items-center text-white animate-blinkV2 font-medium px-4" style={{ transform: "skewX(-45deg)" }}>
              {text}
            </div>
          </div>
        </>
