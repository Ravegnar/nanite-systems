...
const StyledButton = ({
	...
	useEffect(() => {
		...
		ctx.lineTo(right - backgroundOffset, offsetedBottom - corner - backgroundOffset / 2)
		ctx.lineTo(right - backgroundOffset / 2 - corner, offsetedBottom - backgroundOffset)
		ctx.lineTo(left + backgroundOffset, offsetedBottom - backgroundOffset)
		ctx.lineTo(left + backgroundOffset, offsetedTop + corner + backgroundOffset / 2)
		ctx.lineTo(left + backgroundOffset / 2 + corner, offsetedTop + backgroundOffset)
		ctx.lineTo(right - backgroundOffset + lineOffset, offsetedTop + backgroundOffset)
		// Draw the Path
	...
	return (
		<>
			...
			>
				...
				{index ? (
					<span className="font-orbitron text-white dark:text-orange-500X font-bold">0{index}</span>
				) : null}
				<canvas
					ref={canvas}
