...
export const Scroller = () => {
	...
	const lineOffsetY = (scrollerMid - (scrollerOffsetY + scrollerWidth * 2 + scrollerWidth)) / 3.5
	const spotSize = 28
	const spotTop = 10 + scrollerOffsetY + scrollerWidth * 2 + scrollerWidth - spotSize / 2
	const spotBottom =
		-10 + scrollerHeight - scrollerOffsetY - scrollerWidth * 2 - scrollerWidth - spotSize / 2
	...
	const handleMouseMove = (e: any) => {
		...
		if (isDragging) {
			let spotPos = (spot.current.getBoundingClientRect().y - spotTop + 5.79) / spotP
			let scrollTo = (scrollMaxHeight / 100) * spotPos
			let mousePos = e.clientY - spotSize / 2

			mousePos = mousePos <= spotTop ? spotTop : mousePos >= spotBottom ? spotBottom : mousePos
	...
	useEffect(() => {
		...
		ctx.lineTo(scrollerLeft, scrollerOffsetY + scrollerWidth * 2 + scrollerWidth)
		ctx.lineTo(scrollerLeft, scrollerMid + 4 - scrollerWidth)
		ctx.lineTo(scrollerRight - 4, scrollerMid)
		ctx.lineTo(scrollerLeft, scrollerMid - 4 + scrollerWidth)
		ctx.lineTo(scrollerLeft, scrollerHeight - scrollerOffsetY - scrollerWidth * 2 - scrollerWidth)
		ctx.lineTo(scrollerRight, scrollerHeight - scrollerOffsetY - scrollerWidth * 2)
		...
		ctx.lineTo(scrollerLeft + 8, scrollerOffsetY + scrollerWidth * 2 + scrollerWidth + 2)
		...
		ctx.lineTo(scrollerRight + 12, scrollerHeight - scrollerOffsetY - scrollerWidth * 2 + 2.5)
		...
		for (let i = 0; i <= 6; i++) {
			...
			for (let index = 0; index <= 9; index++) {
				...
				if ((i === 0 && index === 9) || (i === 3 && index === 0) || (i === 6 && index === 0)) {
					...
					linePosY = i === 3 ? scrollerMid - lineOffsetY * 2.5 + 6 * lineOffsetY : linePosY
	...
	useEffect(() => {
		...
		return () => {
			;(isScrolling = false), window.removeEventListener("mousemove", handleMouseMove)
